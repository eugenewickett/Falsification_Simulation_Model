...
nodeInputFileString = 'NSF_Nodes.csv' 
arcPreferencesFileString = 'NSF_Arcs_Prefs.csv' 
arcLTsFileString = 'NSF_LTs.csv' 
arcRsFileString = 'NSF_Arcs_Rs.csv' 
...
diagnosticSensitivity = 1 # Tool sensitivity 
diagnosticSpecificity = 1 # Tool specificity 
...
for rep in range(numReplications): 
    ...
    ### SHORTEN LIST SIZES HERE 
    intermediateNum = 3 
    endNum = 25 
     
    for i in range(10-intermediateNum): 
        currIntermediate = List_IntermediateNode[-1] 
        List_IntermediateNode.remove(currIntermediate) 
    for i in range(106-endNum): 
        currEnd = List_EndNode[-1] 
        List_EndNode.remove(currEnd) 
     
    for indEnd in range(endNum): 
        currEnd = List_EndNode[indEnd] 
        newPrefsList = [x for x in currEnd.PreferenceList if x<(intermediateNum+2)] 
        if newPrefsList == [1]: 
            newInt = np.random.random_integers(2,2+intermediateNum-1) 
            newPrefsList = [newInt,1] 
        currEnd.PreferenceList = newPrefsList 
     
     
    for indInt in range(intermediateNum): 
        currIntermediate = List_IntermediateNode[indInt] 
        currIntermediate.r = 150 
    ...
    #for indEnd in range(endNum): 
    #    currEnd= List_EndNode[indEnd] 
    #    mainPref = np.mod(indEnd,5) + 2 
    #    randVec = np.random.permutation([2,3,4,5,6]).tolist() 
    #    randVec.remove(mainPref) 
    #    currEnd.PreferenceList = [mainPref]+randVec+[1] 
    #    currEnd.PreferenceLTsList = [2,2,2,2,2,1] 
    #    currEnd.PreferenceRsList = [500,500,500,500,500,40] 
    ...
    #    currEnd = List_EndNode[indEnd] 
    #    mainPref = np.mod(indEnd,5) + 2 
    #    secondaryPref = np.mod(indEnd+1,5) + 2 
    #    currEnd.PreferenceList = [mainPref,secondaryPref,1] 
    #    currEnd.PreferenceLTsList = [2,2,1] 
    #    currEnd.PreferenceRsList = [500,500,40] 
    ...
    List_TestingSchedule = [testRow for testRow in List_TestingSchedule if testRow[1] < 12+endNum] 
                   
    #Node 5 is bad 
    List_IntermediateNode[2].FalsifierProbability = 0.2 
