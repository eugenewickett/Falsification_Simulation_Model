...
import SFP_Sim_Classes as simClasses # modules for the simulation
...
def SFPSim_EvaluateIntervals(OPdicts,dictNamesVec=[],threshold=0.2):
    ...
    ratses, for a LIST of output dictionaries.
...
def SimSFPEstimateOutput(OPdicts,dictNamesVec=[],threshold=0.2):
    ...
    for currDict in OPdicts:
        ...
        for repNum in currDict.keys():
            ...
            if hasPost:
                currPostdevs = [np.median(sps.expit(currPostsamples[:,i]))-currTrueSFVec[i] for i in range(len(currTrueSFVec))]
            if hasPosttr:
                currPosttrdevs = [np.median(sps.expit(currPosttrsamples[:,i]))-currTrueSFVec[i] for i in range(len(currTrueSFVec))]
            ...
            if hasPost:
                currPostAbsdevs = [np.abs(np.median(sps.expit(currPostsamples[:,i]))-currTrueSFVec[i]) for i in range(len(currTrueSFVec))]
            if hasPosttr:
                currPosttrAbsdevs = [np.abs(np.median(sps.expit(currPosttrsamples[:,i]))-currTrueSFVec[i]) for i in range(len(currTrueSFVec))]
            ...
            currPostProj_Bin = [1 if np.median(sps.expit(currPostsamples[:,i])) > threshold else 0 for i in range(len(currTrueSFVec))]
            currPosttrProj_Bin = [1 if np.median(sps.expit(currPosttrsamples[:,i])) > threshold else 0 for i in range(len(currTrueSFVec))]
        ...
        # Loop through each replication contained in the current output dictionary
            ...
            for repNum in currDict.keys():
                ...
                if not not currScenInds: #Only find deviations if the scenario was used (list is nonempty)
                    ...
                    if hasPost:
                        currScenPostdevs = [np.median(sps.expit(currPostsamples[:,currScenInds[i]]))-currScenTrueSFVec[i] for i in range(len(currScenTrueSFVec))]
                    if hasPosttr:
                        currScenPosttrdevs = [np.median(sps.expit(currPosttrsamples[:,currScenInds[i]]))-currScenTrueSFVec[i] for i in range(len(currScenTrueSFVec))]
                    ...
                    if hasPost:
                        currScenPostAbsdevs = [np.abs(np.median(sps.expit(currPostsamples[:,currScenInds[i]]))-currScenTrueSFVec[i]) for i in range(len(currScenTrueSFVec))]
                    if hasPosttr:
                        currScenPosttrAbsdevs = [np.abs(np.median(sps.expit(currPosttrsamples[:,currScenInds[i]]))-currScenTrueSFVec[i]) for i in range(len(currScenTrueSFVec))]
    ...
    for currDict in OPdicts:
        ...
        for repNum in currDict.keys():
            ...
            currMLEtrProjdevs = [currMLEtrProj[i]-currTrueSFVec[i] for i in range(len(currTrueSFVec)) if not np.isnan(currMLEtrProj[i])]
            if hasPost:
                currPostdevs = [np.median(sps.expit(currPostsamples[:,i+numInts]))-currTrueSFVec[i] for i in range(len(currTrueSFVec))]
            if hasPosttr:
                currPosttrdevs = [np.median(sps.expit(currPosttrsamples[:,i+numInts]))-currTrueSFVec[i] for i in range(len(currTrueSFVec))]
            ...
            currMLEtrProjAbsdevs = [np.abs(currMLEtrProj[i]-currTrueSFVec[i]) for i in range(len(currTrueSFVec))]
            if hasPost:
                currPostAbsdevs = [np.abs(np.median(sps.expit(currPostsamples[:,i+numInts]))-currTrueSFVec[i]) for i in range(len(currTrueSFVec))]
            if hasPosttr:
                currPosttrAbsdevs = [np.abs(np.median(sps.expit(currPosttrsamples[:,i+numInts]))-currTrueSFVec[i]) for i in range(len(currTrueSFVec))]
            ...
            currMLEProj_Bin = [1 if currMLEProj[i] > threshold else 0 for i in range(len(currTrueSFVec))]
            currMLEtrProj_Bin = [1 if currMLEtrProj[i] > threshold else 0 for i in range(len(currTrueSFVec))]
            currPostProj_Bin = [1 if np.median(sps.expit(currPostsamples[:,i+numInts])) > threshold else 0 for i in range(len(currTrueSFVec))]
            currPosttrProj_Bin = [1 if np.median(sps.expit(currPosttrsamples[:,i+numInts])) > threshold else 0 for i in range(len(currTrueSFVec))]
            # Generate true/false positives/negatives rates, plus accuracy
            if len([i for i in range(len(currLinProj_Bin)) if (currLinProj_Bin[i] == 1)]) > 0:
                currDict_truePos_Lin.append(np.sum([currTrueSFVec_Bin[i] for i in range(len(currLinProj_Bin)) if (currLinProj_Bin[i] == 1) ])/\
                                            len([i for i in range(len(currLinProj_Bin)) if (currLinProj_Bin[i] == 1)]))
    ...
    for dictInd,currDict in enumerate(dictNamesVec):
        ...
        block4 = list(zip(itertools.cycle([currDict]),\
                          itertools.cycle(['Posterior sample Medians']),\
