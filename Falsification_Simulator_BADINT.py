...
for rep in range(numReplications):
    ...
    List_TestingSchedule = simModules.testingScheduleGenerator(nodes=nodeList, int_numDays=NumSimDays, int_sampleBudget = samplingBudget, int_PolicyType = 2, arr_PolicyParameter = [0])
    TestReportTbl = []
    ...
    currIntermediate = List_IntermediateNode[int(np.floor(np.random.uniform(0,intermediateNum)))]
    ...
    for today in range(NumSimDays):
        ...
        if today == NumSimDays-1 and np.mod(rep,10)==9: # For updating while running
    ...
    if printOutput == True: # Printing of tables and charts
        ...
        plt.savefig(fileAppend+'_root consumption.png')
        plt.show()
        ...
        plt.savefig(fileAppend+'_int stockout.png')
        plt.show()
        
        # End stockout %'s
        fig = plt.figure()
        ax = fig.add_axes([0.1,0.1,3,0.5])
        ax.set_xlabel('End Node',fontsize=16)
        ax.set_ylabel('Percentage stocked out',fontsize=16)
        #vals = ax.get_yticks()
        #ax.set_yticklabels(['{:,.0%}'.format(x) for x in vals])
        ax.bar(End_Plot_x,End_Plot_y)
        plt.savefig(fileAppend+'_end stockout.png')
        plt.show()
        ...
        plt.savefig(fileAppend+'_testing.png')
        plt.show()
...
if warmUpRun == False and generateOutput == True:
