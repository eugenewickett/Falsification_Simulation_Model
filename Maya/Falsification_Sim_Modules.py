...
def generateNodeListsFromFile(nodeInputFileString='',
    ...
    for currNode in range((len(nodeList))):
        ...
        if currRootTF == 'TRUE': # Currently a root node
            ...
            for indSupplier in range(numSuppliers): # Add these suppliers by rank  #*MZ: are there any tie rankings?
...
def testingScheduleGenerator(nodes = [], int_numDays=1000,
    ...
    if int_PolicyType == 0: # Deterministic End-Node policy #one end node a day
        ...
        #MZ: determine  sample schedule based on # days. 
    
    elif int_PolicyType == 1: # Deterministic End-Node policy with multiple tests per day, set via int_sampleBudget  #can test multiple end nodes a day dependent on budget
        ...
        #MZ: determine  sample schedule based on sampling budget. 
        #MZ*:  what if ur  budget is really small and you can't get to every endnode? Should suppliers have  a preference on who they supply to as well?
